import {
  EmailParams,
  sendPlaySessionParticipantLeft,
} from 'services/server/communications/email/sendPlaySessionParticipantLeft';
...
const send = async (items: EmailParams[]) => {
  const sendPromises = items.map((item) => sendPlaySessionParticipantLeft(item));

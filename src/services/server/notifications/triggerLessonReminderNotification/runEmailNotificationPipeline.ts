...
import {
  Params,
  sendLessonStartReminder,
} from 'services/server/communications/email/sendLessonStartReminder';
...
const PREFERENCE_KEY = 'lessonReminderEmail';
...
const transform = async ({ lesson }: CommunicationParams): Promise<NotificationTypedTemplate[]> => {
  ...
  lesson.participants.forEach((lessonParticipant) => {
    if (lesson && lessonParticipant.userId && lessonParticipant.user) {
      notifications.push({
        to: {
...
const send = async (items: NotificationTypedTemplate[]) => {
  const sendPromises = items.map((item) => sendLessonStartReminder(item));

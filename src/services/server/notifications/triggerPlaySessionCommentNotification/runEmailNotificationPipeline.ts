import { sendPlaySessionCommentSubmitted } from 'services/server/communications/email/sendPlaySessionCommentSubmitted';
...
const send = async (items: EmailParams[]) => {
  const sendPromises = items.map((item) => sendPlaySessionCommentSubmitted(item)); // TODO: GET NEW EMAIL TEMPLATE

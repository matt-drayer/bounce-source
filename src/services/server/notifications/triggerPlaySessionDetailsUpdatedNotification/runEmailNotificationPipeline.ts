import { sendPlaySessionDetailsUpdated } from 'services/server/communications/email/sendPlaySessionDetailsUpdated';
...
const send = async (items: EmailParams[]) => {
  const sendPromises = items.map((item) => sendPlaySessionDetailsUpdated(item)); // TODO: GET NEW EMAIL TEMPLATE

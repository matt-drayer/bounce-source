import {
  EmailParams,
  sendLessonAvailableForWaitlist,
} from 'services/server/communications/email/sendLessonAvailableForWaitlist';
...
const send = async (items: EmailParams[]) => {
  const sendPromises = items.map((item) => sendLessonAvailableForWaitlist(item));

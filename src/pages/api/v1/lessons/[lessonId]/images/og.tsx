...
import { LessonTypesEnum } from 'types/generated/server';
...
const extractLessonId = (request: NextRequest) => {
  const pathname = request.nextUrl.pathname.slice(1);
  const lessonId = pathname.split('/')[POSITION_OF_ID_IN_URL];

  return lessonId;
};

const getBadgeColor = (lessonType: LessonTypesEnum) => {
  if (lessonType === LessonTypesEnum.Cardio) {
    return palette.lessonCardio;
  } else if (lessonType === LessonTypesEnum.Camp) {
    return palette.lessonCamp;
  } else if (lessonType === LessonTypesEnum.Clinic) {
...
const getBadgeText = (lessonType: LessonTypesEnum) => {
  if (lessonType === LessonTypesEnum.Cardio) {
    return 'CardioTennis';
  } else if (lessonType === LessonTypesEnum.Camp) {
    return 'Tennis Camp';
  } else if (lessonType === LessonTypesEnum.Clinic) {
    return 'Clinic • Tennis';
  } else {
    return 'Private Lesson • Tennis';
  }
};
...
export default async function handler(request: NextRequest) {
  ...
  const lessonId = extractLessonId(request);
  const response = await getLessonById({ id: lessonId });
  const test = request.nextUrl.searchParams.get('lessonId');
  const lesson = response.lessonsByPk;

  if (!lesson) {
    return NextResponse.json({ error: 'Lesson not found' }, { status: 404 });
  }

  const profileImageUrl = lesson?.owner?.profileImagePath
    ? getImageUrl(lesson?.owner?.profileImagePath)
    : '';
  const badgeColor = getBadgeColor(lesson?.type);
  const badgeText = getBadgeText(lesson?.type);
  const date = format(new Date(lesson.startDateTime), 'EEEE LLLL d');
  const startLocaleDateString = new Date(lesson.startDateTime).toLocaleString(lesson.locale, {
    timeZone: lesson.timezoneName,
  });
  const endLocaleDateString = new Date(lesson.endDateTime).toLocaleString(lesson.locale, {
    timeZone: lesson.timezoneName,
  });
  ...
  // const offsetMinutes = lesson.timezoneOffsetMinutes || 0;
  // const startTime = format(addMinutes(new Date(lesson.startDateTime), -offsetMinutes), 'p');
  // const endTime = format(addMinutes(new Date(lesson.endDateTime), -offsetMinutes), 'p');
  const courtName = lesson.userCustomCourt?.title || '';
  console.log('---- LESSON ID = ', lessonId, lesson, profileImageUrl);
  console.log({ test });
        ...
        <div tw="flex flex-col w-full">
          <div tw="flex justify-between w-full">
            <div tw="flex flex-col">
              ...
              <h1
                tw="font-bold mt-6"
                style={{ fontFamily: '"Inter"', fontSize: '48px', lineHeight: 1, display: 'flex' }}
              >
                {lesson.title}
              </h1>
              <h2
              ...
              >
                {date} @ {startTime} - {endTime} {lesson.timezoneAbbreviation || ''}
              </h2>
            </div>
            {!!profileImageUrl && (
              <img
          ...
          <div tw="mt-8 flex text-2xl text-gray-600" style={{ width: '800px', lineHeight: 1.65 }}>
            {lesson.description}
          </div>
        </div>
        <div tw="flex justify-between items-center w-full">
          <div tw="flex text-2xl items-center">
            <span tw="font-bold">Coach: {lesson.owner?.fullName}</span>{' '}
            {/* <span tw="text-gray-600 ml-2">@{lesson.owner?.username}</span> */}
          </div>
          <div tw="flex">

...
import { PlaySessionFormatsEnum, SportsEnum } from 'types/generated/server';
...
const extractPlaySessionId = (request: NextRequest) => {
  const pathname = request.nextUrl.pathname.slice(1);
  const playSessionId = pathname.split('/')[POSITION_OF_ID_IN_URL];

  return playSessionId;
};

const getBadgeColor = (playSessionFormat: PlaySessionFormatsEnum) => {
  if (playSessionFormat === PlaySessionFormatsEnum.Singles) {
    return palette.lessonCardio;
  } else if (playSessionFormat === PlaySessionFormatsEnum.Doubles) {
    return palette.lessonCamp;
  } else if (playSessionFormat === PlaySessionFormatsEnum.OpenPlay) {
    return palette.lessonClinic;
  } else {
    return palette.lessonIndividual;
  }
};

const getBadgeText = (playSessionFormat: PlaySessionFormatsEnum) => {
  if (playSessionFormat === PlaySessionFormatsEnum.Singles) {
    return 'Singles';
  } else if (playSessionFormat === PlaySessionFormatsEnum.Doubles) {
    return 'Doubles';
...
export default async function handler(request: NextRequest) {
  ...
  const lessonId = extractPlaySessionId(request);
  const response = await getPlaySessionById({ id: lessonId });
  const test = request.nextUrl.searchParams.get('lessonId');
  const playSession = response.playSessionsByPk;

  if (!playSession) {
    return NextResponse.json({ error: 'Play session not found' }, { status: 404 });
  }

  const profileImageUrl = playSession?.organizer?.profileImagePath
    ? getImageUrl(playSession?.organizer?.profileImagePath)
    : '';
  const badgeColor = getBadgeColor(playSession?.format);
  const badgeText = getBadgeText(playSession?.format);
  const date = format(new Date(playSession.startDateTime), 'EEEE LLLL d');
  const startLocaleDateString = new Date(playSession.startDateTime).toLocaleString(
    playSession.locale,
    {
      timeZone: playSession.timezoneName,
    },
  );
  const endLocaleDateString = new Date(playSession.endDateTime).toLocaleString(playSession.locale, {
    timeZone: playSession.timezoneName,
  });
  const startLocaleDate = new Date(startLocaleDateString);
  const endLocaleDate = new Date(endLocaleDateString);
  const startTime = format(startLocaleDate, 'p');
  const endTime = format(endLocaleDate, 'p');
  // const offsetMinutes = playSession.timezoneOffsetMinutes || 0;
  // const startTime = format(addMinutes(new Date(playSession.startDateTime), -offsetMinutes), 'p');
  // const endTime = format(addMinutes(new Date(playSession.endDateTime), -offsetMinutes), 'p');
  const courtName = playSession.venue?.title || '';
  console.log('---- PLAY SESSION ID = ', lessonId, playSession, profileImageUrl);
  console.log({ test });
    ...
    (
      ...
      >
          ...
          <div tw="flex justify-between w-full">
            <div tw="flex flex-col">
              <div tw="flex items-center">
                <div tw="rounded-[64px] px-5 py-3 flex" style={{ backgroundColor: badgeColor }}>
                  {badgeText}
                </div>
                <div tw="ml-4 flex items-center">
                  {!playSession.targetSkillLevel && (
                    <div tw="flex px-5 py-3 items-center rounded-[64px] bg-[#EFF9DB] text-[#155E75]">
                      All Skill Levels
                    </div>
                  )}
                  ...
                  {playSession.targetSkillLevel === SkillLevels.Beginner && (
                    <div tw="flex px-5 py-3 items-center rounded-[64px] bg-[#F0F9FF] text-[##316BDA]">
                      Beginner
                    </div>
                  )}
                  {playSession.targetSkillLevel === SkillLevels.Intermediate && (
                    <div tw="flex px-5 py-3 items-center rounded-[64px] bg-[#FBF8E5] text-[#AF7400]">
                      Intermediate
                    </div>
                  )}
                  {playSession.targetSkillLevel === SkillLevels.Advanced && (
                    <div tw="flex px-5 py-3 items-center rounded-[64px] bg-[#FFECDE] text-[#ED4F2F]">
                      Advanced
                    </div>
                  )}
                  {playSession.targetSkillLevel === SkillLevels.Pro && (
                    <div tw="flex px-5 py-3 items-center rounded-[64px] bg-[#FFEEF4] text-[#86175A]">
                      Pro
                    </div>
                  )}
                </div>
                <div tw="flex items-center ml-4 font-semibold">
                  {playSession.sport === SportsEnum.Tennis && (
                    <div tw="flex items-center">Tennis</div>
                  )}
                  {playSession.sport === SportsEnum.Pickleball && (
                    <div tw="flex items-center">Pickleball</div>
                  )}
                </div>
              </div>
              {!!playSession.title && (
              ...
              >
                {date} @ {startTime} - {endTime} {playSession.timezoneAbbreviation || ''}
              </h2>
              <h3 tw="mt-0 flex items-center">
                <svg viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path
                    fill-rule="evenodd"
                    clip-rule="evenodd"
                    d="M4.04059 3.24018C6.22752 1.05324 9.77324 1.05324 11.9602 3.24018C14.1471 5.42711 14.1471 8.97284 11.9602 11.1598L8.00038 15.1196L4.04059 11.1598C1.85366 8.97284 1.85366 5.42711 4.04059 3.24018ZM8.00038 8.79998C8.88404 8.79998 9.60038 8.08363 9.60038 7.19998C9.60038 6.31632 8.88404 5.59998 8.00038 5.59998C7.11673 5.59998 6.40038 6.31632 6.40038 7.19998C6.40038 8.08363 7.11673 8.79998 8.00038 8.79998Z"
                    fill="#A09FA5"
                  />
                </svg>
                <span tw="ml-1" style={{ color: '#A09FA5' }}>
                  {courtName}
                </span>
              </h3>
              <div
                tw="mt-8 flex text-2xl text-gray-600"
        ...
        <div tw="flex justify-between items-center w-full">
          <div tw="flex text-2xl items-center">
            <span tw="font-bold">Organizer: {playSession.organizer?.fullName}</span>{' '}
            {/* <span tw="text-gray-600 ml-2">@{playSession.organizer?.username}</span> */}
          </div>
          <div tw="flex">

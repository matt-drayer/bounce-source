...
import { type NextApiRequestWithViewerRequired } from 'utils/server/edge/middleware/withViewerDataRequired';
import { withViewerDataRequired } from 'utils/server/edge/middleware/withViewerDataRequired';
...
const POST = async (req: NextApiRequestWithViewerRequired) => {
  try {
    ...
    const responseData = await uploadImage(payload.get('sponsor') as File);
